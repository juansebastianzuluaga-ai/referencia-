      ...
      <el-tab-pane name="profile">
        ...
        <div class="max-w-3xl mx-auto" v-loading="loading">
          ...
          <div class="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
            <div class="w-16 h-16 rounded-full bg-[var(--blue-800)] flex items-center justify-center shrink-0">
              <UserIcon class="w-8 h-8 text-white" />
            </div>
            <div class="min-w-0">
              <h3 class="text-lg font-semibold text-gray-900 truncate">{{ auth.user?.full_name || 'Usuario' }}</h3>
              <p class="text-sm text-gray-500 truncate">{{ auth.user?.job_title || 'Personal' }}</p>
              <div class="flex flex-wrap gap-1 mt-1">
                <el-tag v-for="role in auth.user?.roles" :key="role.name" size="small" type="info">
                  {{ role.name }}
                </el-tag>
              </div>
            </div>
